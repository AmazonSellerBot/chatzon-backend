...
def update_listing(req: ListingUpdateRequest):
    try:
        ...
        return {"status": "success", "type": "listing_update", "asin": req.asin, "field": req.field, "value": req.value, "feedId": feed_id}
...
def update_price_fast(req: PriceUpdateFastRequest):
    try:
        ...
        feed_data = [{
            "sku": req.sku,
            "standard_price": {"currency": "USD", "amount": str(req.new_price)}
        }]
        ...
        return {"status": "success", "type": "price_update", "asin": req.asin, "sku": req.sku, "new_price": req.new_price, "feedId": feed_id}
    except Exception as e:
        return JSONResponse(status_code=500, content={"status": "error", "message": str(e)})
    ...
    try:
        ...
        if response.status_code != 200:
            return JSONResponse(status_code=502, content={
                "status": "error",
                "message": f"Amazon returned status code {response.status_code}"
            })
        ...
        if not title_el:
            return JSONResponse(status_code=404, content={"status": "error", "message": "Could not extract title"})
        ...
        return JSONResponse(status_code=504, content={"status": "error", "message": "Request to Amazon timed out."})
    except Exception as e:
        return JSONResponse(status_code=500, content={"status": "error", "message": str(e)})
