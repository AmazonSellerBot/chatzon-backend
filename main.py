# main.py
# Chatzon Backend – Catalog+Listings+PriceUpdate (sellerId fix + robust pricing)
# Routes:
#   GET  /health
#   GET  /catalog-by-sku
#   GET  /inspect-listing
...
def _sign(method: str, host: str, region: str, uri: str, qs: str,
    ...
    signed_header_keys = sorted(ch.keys())
    chs="".join([f"{k}:{ch[k]}\n" for k in signed_header_keys])
    sh=";".join(signed_header_keys)
    ...
    out={"x-amz-date": amz,
         "Authorization": f"{alg} Credential={ak}/{scope}, SignedHeaders={sh}, Signature={sig}"}
...
def _exec(method: str, path: str, query: Optional[Dict[str,Any]] = None, body: Optional[Dict[str,Any]] = None) -> requests.Response:
    ...
    signed=_sign(method, host, AWS_REGION, path, qs,
                 {"x-amz-access-token": headers["x-amz-access-token"], "content-type": headers["content-type"]},
                 payload, ak, sk, st)
    hdr={**headers, **signed}

    m=method.upper()
    if m=="GET":    return requests.get(url, headers=hdr, params=query, data=None, timeout=60)
...
# ===== App =====
app = FastAPI(title="Chatzon Backend – Catalog+Listings+PriceUpdate", version="2.8.2")

@app.get("/health")
def health():
    return {"ok": True, "service": "chatzon-backend", "time": datetime.datetime.utcnow().isoformat()+"Z"}

# 1) Catalog lookup by SKU (Amazon requires sellerId for SKU-based searches)
...
def catalog_by_sku(sku: str = Query(...), marketplaceId: str = Query(MARKETPLACE_ID_DEFAULT)):
    resp = _exec(
        ...
        query={
            ...
            "sellerId": SELLER_ID,  # required for SKU searches
...
# 2) Listings Items GET (may reveal productType for this listing)
...
def inspect_listing(sku: str = Query(...), marketplaceId: str = Query(MARKETPLACE_ID_DEFAULT)):
    ...
    # Try to extract productType if present
    pt=None
    try:
...
def _try_patch_price(sku: str, marketplaceId: str, currency: str, amount: float, productType: str):
    ...
    # Try both requirement modes; return first success or first detailed issues
    ...
    # If we got here, nothing succeeded and Amazon gave no detailed issues
...
def update_price(body: Dict[str, Any] = Body(..., example={
    ...
    # Discover productType via Listings GET; fallback to PRODUCT if not returned
