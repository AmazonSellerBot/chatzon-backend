...
REQ_ENVS = [
    ...
    "SELLER_ID","REGION","SP_API_ENDPOINT"
]
for k in REQ_ENVS:
    if not os.getenv(k):
...
def _get_signature_key(key: str, date_stamp: str, region_name: str, service_name: str) -> bytes:
    ...
    import hmac as _h, hashlib as _hs
    k_region = _h.new(k_date, region_name.encode("utf-8"), _hs.sha256).digest()
    k_service = _h.new(k_region, service_name.encode("utf-8"), _hs.sha256).digest()
    k_signing = _h.new(k_service, b"aws4_request", _hs.sha256).digest()
    return k_signing
...
def build_json_listings_price_feed(sku: str, marketplace_id: str, amount: float, currency: str = "USD") -> str:
    message = {
        ...
        "productType": "PRODUCT",
        "patches": [
            {"op":"replace","path":"/attributes/standard_price","value":[{"currency":currency,"amount":round(float(amount),2)}]}
        ]
    }
    return json.dumps({"header":{"sellerId":SELLER_ID,"version":"2.0"},"messages":[message]}, separators=(",",":"))
...
def update_price_fast(payload: PriceUpdatePayload):
    ...
    upload_feed_to_s3(doc["url"], feed_body)
    ...
    return PriceUpdateResponse(
        ...
        note="Use /feed-status?feedId=... until DONE, or call /feed-report-by-doc?docId=..."
    )
...
def feed_report(feedId: str = Query(...)):
    ...
    doc_id = info.get("resultFeedDocumentId")
    if not doc_id:
